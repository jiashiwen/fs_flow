...
use crate::{
    ...
    commons::{create_parent_dir, scan_folder_files_to_multi_files, RegexFilter},
    consts::task_consts::{COMPARE_RESULT_PREFIX, OFFSET_PREFIX},
    models::{
        model_checkpoint::{FileDescription, FilePosition},
        model_task_compare::{CompareCheckOption, CompareTaskAttributes},
    },
    tasks::{
        gen_file_path,
...
impl CompareTaskActions for TaskCompareLocal2Local {
    async fn list_objects_to_multi_files(&self, meta_dir: &str) -> Result<Vec<FileDescription>> {
        let regex_filter: Option<RegexFilter> =
